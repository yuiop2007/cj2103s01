...
<script>
	...
	function updCheck() {
		var pwd = contentform.ePwd.value;
		if(pwd == "") {
			alert("비밀번호를 입력하세요")
			contentform.ePwd.focus();
		}
		else {
			contentform.submit();
		}
	...
	function delCheck() {
		...
		if(ans) {
			if(pwd == "") {
				...
				$.ajax({
					type : "post",
					url  : "${ctp}/board/eDelete?idx=${vo.eId}&ePwd="+pwd+"&pag=${pag}&pageSize=${pageSize}",
					success: function(data) {
						alert("삭제처리 되었습니다.");
						location.href="${ctp}/board/event?pag=${pag}&pageSize=${pageSize}";
					}
				});
			}
		}
			...
			<div class=board>
				...
				<div class=boardbtn>
					...
					<a href="#" onclick="delCheck()">삭제</a>
					  ...
					  <input type="hidden" name="idx" value="${vo.eId}"/>
			</div>
		</form>
	</div>
	<div class="jumbotron">
