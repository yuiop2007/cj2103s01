...
<head>
  ...
  <script>
  	...
  	function suCheck(idx) {
  		var totalPrice = $("#price"+idx).val() * $("#su"+idx).val();
  		//document.getElementById("totalPrice"+idx).value = numberWithCommas(totalPrice);
		    ...
		    <tr>
		      ...
		      <td align="left">
		        <p >
		          ...
		          <br/>금액 : <fmt:formatNumber value="${vo.price}"/>원 <input type="hidden" name="price${vo.idx}" id="price${vo.idx}" value="${vo.price}"/>
		          <br/>수량 : <input type="number" name="su" id="su${vo.idx}" value="${vo.su}" min="1" size="1" class="text-right" onclick="suCheck(${vo.idx})"/>
		        </p><br/>
		      </td>
		      <td><br/>
		        <%-- <b>총 : <fmt:formatNumber value="${vo.price * vo.su}" pattern='#,###원'/></b><br/><br/> --%>
		        총 : <input type="text" id="totalPrice${vo.idx}" value="${vo.price * vo.su}" size="7"/>
		      </td>
		    </tr>
		    </tbody>
