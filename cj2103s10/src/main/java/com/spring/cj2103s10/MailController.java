...
import com.spring.cj2103s10.vo.MailVO;
...
public class MailController {
	...
	public String pwdConfirmMailFormGet(@PathVariable String toMail, @PathVariable String content) {
		...
		try {
			...
			content += "<p><img src=\"cid:11.jpg\" width='500px'></p><hr>";
			content += "<p>Have a good time!!!</p>";
			content += "<p>접속주소 : <a href='http://218.236.203.146:9090/cj2103s10'>cj2103s10</a></p>";
			messageHelper.setText(content, true);
			// 본문 내부에 그림파일을 함께 보내고자 할때.. 그림을 먼저 저장하고, 저장경로와 함께 파일명을 적는다.
			FileSystemResource file = new FileSystemResource(new File("C:/11.jpg"));
			messageHelper.addInline("11.jpg", file);
	...
	// 아이디 찾은것 전송하기
	@RequestMapping(value = "/idConfirmMailForm/{toMail}/{content}/", method = RequestMethod.GET)
	public String idConfirmMailFormGet(@PathVariable String toMail, @PathVariable String content) {
		String fromMail = "ccbbss1126@gmail.com";
		String title = ">> 아이디를 알려드립니다.";
		String mid = content;
		content = "cj2103s10에서 발송한 메일입니다.\n아래 검색된 아이디로 사이트에 접속하세요\n";
		...
		try {
			...
			// 메세지 내용을 전송한다.(전송전에 필요한 '텍스트+사진' 등에 대한 작업..)
			content = content.replace("\n", "<br>");
			content += "<br><hr><h3>임시비밀번호 : <font color='red'>"+mid+"</font></h3><hr><br>";
			content += "<p><img src=\"cid:11.jpg\" width='500px'></p><hr>";
			content += "<p>Have a good time!!!</p>";
			content += "<p>접속주소 : <a href='http://218.236.203.146:9090/cj2103s10'>cj2103s10</a></p>";
			messageHelper.setText(content, true);
